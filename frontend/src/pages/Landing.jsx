          ...
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
            <div className="relative z-10">
              <h1 className="text-5xl md:text-7xl font-extrabold text-[#191c1d] leading-[1.1] mb-6">
                Luyện phỏng vấn với{' '}
                <span className="bg-gradient-to-r from-[#003fb1] to-[#7127e5] bg-clip-text text-transparent">
                  AI như thật
                </span>
              </h1>
            ...
            <div className="relative">
              <div className="absolute inset-0 bg-[#7127e5]/10 blur-[120px] rounded-full -z-10" />
        ...
        <section className="bg-[#f3f4f5] py-24 px-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {[
                { icon: MessageSquare, title: 'Trả lời lan man', desc: 'Mất 5 phút để giới thiệu bản thân nhưng không đọng lại được giá trị cốt lõi nào cho nhà tuyển dụng.' },
                { icon: Frown, title: 'Bị khớp khi phỏng vấn', desc: 'Run sợ trước những câu hỏi khó, dẫn đến quên sạch kiến thức và kinh nghiệm đã chuẩn bị.' },
                { icon: XCircle, title: 'Feedback chung chung', desc: '"Em rất tiềm năng nhưng chưa phù hợp" - Bạn không bao giờ biết mình thực sự sai ở đâu.' },
              ].map(({ icon: Icon, title, desc }) => (
                <div key={title} className="bg-white p-8 rounded-xl flex flex-col gap-4 shadow-sm">
                  <div className="w-12 h-12 bg-[#ffdad6] text-[#ba1a1a] rounded-full flex items-center justify-center">
                    <Icon className="w-6 h-6" />
                  </div>
                  <h3 className="text-xl font-bold text-[#191c1d]">{title}</h3>
                  <p className="text-[#434654]">{desc}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
              ...
              <div className="md:col-span-8 bg-[#f3f4f5] p-10 rounded-xl flex flex-col justify-center border border-gray-200 shadow-sm">
                ...
                <div className="mt-8 flex gap-4 overflow-hidden">
                  <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-[#003fb1] w-3/4" /></div>
                  <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-[#7127e5] w-1/2" /></div>
                </div>
              </div>
            </div>
          </div>
